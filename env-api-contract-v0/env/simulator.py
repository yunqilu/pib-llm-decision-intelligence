...
Formulas (docs/problem_spec.md §3-4, joint spec §2-3):
    ...
    L_t     = sum_i h[i, pow] * rated_power_kw[i]                         # coupling identity (joint spec 3.1)
...
Hard constraints (power cap, thermal redline, SLA floor, action windows) are
enforced by masking (info["action_mask"]), never by reward penalty —
joint spec §3.2, NOI-213 honesty norms. `info["violations"]` must stay
empty; a non-empty list means this module has a bug, not that the policy
misbehaved (masked-out actions are silently ignored, not penalized).
"""
...
class DecisionIntelligenceEnv:
    ...
    def step(self, action: Dict[str, Any]) -> Tuple[Dict[str, Any], Optional[float], bool, bool, Dict[str, Any]]:
        ...
        for p in placements:
            ...
            L_now = float(np.sum(self.H[:, self._pow_idx] * self.rated_power))
            _, F_now, _, _ = self._facility_physics(L_now)
            if F_now + dpow > p_cap + 1e-6:
        ...
        # --- objectives ----------------------------------------------------------
        L_t = float(np.sum(self.H[:, self._pow_idx] * self.rated_power))
    ...
    def _action_mask(self) -> Dict[str, Any]:
        """Hard-constraint mask (joint spec §3.2): power cap, per-host
        multi-resource capacity, admission budget, action windows. Recomputed
        fresh every call from current state."""
        p_cap = self.config["facility"]["p_cap_kw"]
        L_t = float(np.sum(self.H[:, self._pow_idx] * self.rated_power))
        _, F_t, _, _ = self._facility_physics(L_t)
        headroom_kw = max(0.0, p_cap - F_t)
        ...
        for task in self.queue:
            allowed: set = set()
            dpow = task["demand"].get("power", 0.0)
            if dpow <= budget_remaining + 1e-9 and dpow <= headroom_kw + 1e-9:
                delta = self._demand_vector(task)
